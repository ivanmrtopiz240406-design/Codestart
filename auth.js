...
function submitAuth(){
  ...
  const errorBox = document.getElementById("errorBox");
  const agree = document.getElementById("agree"); // 👈 галочка
  ...
  if(!isLogin){
    ...
    if(!agree.checked){
      errorBox.innerText = "Вы должны принять условия перед регистрацией";
      return;
    }
...
function openPrivacy(){
  alert("Политика конфиденциальности:\n\nМы храним только имя, email и прогресс обучения. Эти данные используются исключительно внутри платформы CodeStart и не передаются третьим лицам.");
}

function openTerms(){
  alert("Пользовательское соглашение:\n\nИспользуя платформу CodeStart, вы соглашаетесь использовать материалы только в образовательных целях. Администрация не несёт ответственности за последствия неправильного использования материалов.");
}
